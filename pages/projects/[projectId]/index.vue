...
const deleteAnimation = (animation) => {
    confirm.require({
        ...
        accept: () => {
            project.value.animations = project.value.animations.filter(item => item.id !== animation.id)
            console.log(project.value.animations);

            $fetch(`/api/projects/${project.value.id}`, {
                method: 'POST',
